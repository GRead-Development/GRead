...
function gread_register_rest_routes() {
    ...
    register_rest_route('gread/v1', '/user/unblock', array(
        'methods' => 'POST', // Using POST for consistency, though DELETE could be used
...
// --- Existing API Callback Functions ---
...
function gread_update_reading_progress($request) {
    ...
    if ($current_page < 0 || ($total_pages > 0 && $current_page > $total_pages)) {
        // Allow setting page count higher than max, just cap it.
        $current_page = ($total_pages > 0) ? $total_pages : $current_page;
    }
...
function gread_get_user_stats($request) {
    ...
    if ($user_id !== $current_user_id && !current_user_can('manage_options')) {
        // Allow any logged-in user to view basic stats of another user
