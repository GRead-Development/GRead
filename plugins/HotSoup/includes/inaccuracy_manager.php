...
function hs_add_report_modal_to_book_pages()
{
	...
	<button id="hs-open-report-modal" class="hs-button">Report Inaccuracy</button>
...
function hs_add_report_button_to_content($content)
{
	if (is_singular('book') && in_the_loop() && is_main_query())
	{
		$button_html = '<button id="hs-open-report-modal" class="hs-button">Report Inaccuracy</button>';
		$content .= $button_html;
	}

	return $content;
}
add_filter('the_content', 'hs_add_report_button_to_content');
